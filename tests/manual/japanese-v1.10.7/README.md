# ExcelMcp v1.10.7 日本語対応手動検証

本家 v1.10.5〜v1.10.7 で追加された新機能（What-If Analysis、QueryTable、XmlMap、Drawing、Workbook SaveAs 等）が日本語環境で正しく動作するかを検証する手動テスト手順です。

## 検証環境

- Windows 11
- Microsoft Excel 2016 以降
- ExcelMcp CLI v1.10.7-ja.1 以降
...
|---|---|---|
| 1 | 新規ブック作成 | 日本語ファイルパス |
| 2 | シート作成 | 日本語シート名 `売上データ` |
| 3 | QueryTable 作成 | 日本語 CSV、日本語 QueryTable 名 `売上クエリ` |
| 4 | XmlMap 追加 | 日本語 XSD、日本語 Map 名 `商品マップ`、日本語要素名 |
| 5 | Scenario 作成 | 日本語 Scenario 名 `楽観シナリオ` |
| 6 | Drawing（画像追加） | 日本語ファイルパスの BMP、日本語名 `ロゴ画像` |
| 7 | SaveAs | 日本語出力ファイルパス |
...
$assets = "C:\Users\avalo\projects\EXCELMCP日本語対応\mcp-server-excel\tests\manual\japanese-v1.10.7\assets"
